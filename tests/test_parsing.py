...
def test_parser():
    ...
    pprint(parsed[1].rooms[0].__dict__)

    tools.assert_equal(b'M:',        parsed[1].msg_type)
    tools.assert_equal('00',         parsed[1].unknown)
    tools.assert_equal('01',         parsed[1].unknown2)
    tools.assert_equal(2,            parsed[1].version)
    tools.assert_equal(86,           parsed[1].magicbyte)
    tools.assert_equal(1,            parsed[1].unknown3)

    tools.assert_equal(3,            len(parsed[1].devices))

    tools.assert_equal(1,                parsed[1].devices[0].room_id)
    tools.assert_equal(b'0b04be',        parsed[1].devices[0].rf_address)
    tools.assert_equal('KEQ0571674',     parsed[1].devices[0].serial)
    tools.assert_equal(2,                parsed[1].devices[0].type)
    tools.assert_equal('topeni u wc',    parsed[1].devices[0].name)
    
    tools.assert_equal(2,                parsed[1].devices[1].room_id)
    tools.assert_equal(b'08b6d2',        parsed[1].devices[1].rf_address)
    tools.assert_equal('KEQ0634607',     parsed[1].devices[1].serial)
    tools.assert_equal(2,                parsed[1].devices[1].type)
    tools.assert_equal('Pod oknem',      parsed[1].devices[1].name)

    tools.assert_equal(0,                parsed[1].devices[2].room_id)
    tools.assert_equal(b'01b491',        parsed[1].devices[2].rf_address)
    tools.assert_equal('JEQ0305205',     parsed[1].devices[2].serial)
    tools.assert_equal(5,                parsed[1].devices[2].type)
    tools.assert_equal('Eco Switch',     parsed[1].devices[2].name)


    tools.assert_equal(2,            len(parsed[1].rooms))

    tools.assert_equal(2,                parsed[1].rooms[0].id)
    tools.assert_equal(b'08b6d2',        parsed[1].rooms[0].rf_address)
    tools.assert_equal('Obyvaci pokoj',  parsed[1].rooms[0].name)

    tools.assert_equal(1,                parsed[1].rooms[1].id)
    tools.assert_equal(b'0b04be',        parsed[1].rooms[1].rf_address)
    tools.assert_equal('Predsin',        parsed[1].rooms[1].name)
